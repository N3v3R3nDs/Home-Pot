...
export function ChipsPage() {
  ...
  return (
    <div className="space-y-4 pb-4">
      <header>
        <h1 className="font-display text-3xl text-brass-shine">🎰 Tonight's chips</h1>
        <p className="text-ink-400 text-sm mt-1">
          {isTracking
            ? `Tracking live game · auto-updates as ${liveTournament ? 'players bust' : 'cash players join'}`
            : 'Plan the dealer\'s float across tournament + cash so you don\'t run out of any denomination.'}
        </p>
      </header>
        ...
        <Card className="bg-brass-500/5 border border-brass-500/30">
          <div className="flex items-center justify-between gap-3">
            <span className="flex items-center gap-2 text-brass-100 text-sm">
              ...
              <span>
                <div className="font-semibold">Tracking live game</div>
                <div className="text-[11px] text-ink-400">
                  {liveTournament && `${liveTournament.name} · ${livePlayers.length - bustedCount}/${livePlayers.length} alive`}
                  {liveTournament && liveCash && ' · '}
                  {liveCash && `Cash: ${liveCash.name}`}
                </div>
              </span>
            </span>
      ...
      <Card>
        ...
        <p className="label mt-4">Cash game</p>
        <div className="grid grid-cols-2 gap-3">
          <NumberInput
            label="Buy-in"
            value={cBuyIn}
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span>
                <div className="font-display text-emerald-200 text-lg">Tournament reserve</div>
                <div className="text-[11px] text-ink-400">
                  {plan.tournament.activePlayers} active stack{plan.tournament.activePlayers === 1 ? '' : 's'}
                  {bustedCount > 0 && isTracking && ` · ${bustedCount} returned to dealer`}
                </div>
              </span>
            </div>
            <div className="text-right">
              ...
              <div className="text-[10px] text-ink-400">total reserved</div>
            </div>
          </div>

          <div className="bg-felt-950/40 rounded-xl p-3">
            <div className="text-[10px] uppercase tracking-widest text-ink-400 mb-2">Per-player stack</div>
            <ChipBag entries={plan.tournament.perPlayer} />
            <div className="mt-2 text-xs text-ink-400">
              Target {formatChips(plan.tournament.perPlayerTargetValue)} · Built {formatChips(plan.tournament.perPlayerActualValue)}
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-2xl">💵</span>
              <span>
                <div className="font-display text-brass-200 text-lg">Cash buy-in bag</div>
                <div className="text-[11px] text-ink-400">
                  Sized for {formatChips(cBuyIn)} buy-in · {cSmall}/{cSmall * 2} blinds
                </div>
              </span>
            </div>
            <div className="text-right">
              ...
              <div className="text-[10px] text-ink-400">buy-in{plan.cash.buyInsAvailable === 1 ? '' : 's'} available</div>
            </div>
          </div>

          <div className="bg-felt-950/40 rounded-xl p-3">
            <div className="text-[10px] uppercase tracking-widest text-ink-400 mb-2">Per buy-in</div>
            <ChipBag entries={plan.cash.perBuyIn} />
            <div className="mt-2 text-xs text-ink-400">
              Target {formatChips(plan.cash.perBuyInTargetValue)} · Built {formatChips(plan.cash.perBuyInActualValue)}
            </div>
          </div>
        ...
        <Card className="border border-red-500/40 bg-red-500/5">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm">
              <div className="font-semibold text-red-300">Inventory short</div>
              <div className="text-[12px] text-ink-300 mt-1">
                Reserving the tournament + 1 cash bag would over-commit:
              </div>
              <ul className="mt-1 space-y-0.5 text-xs text-red-300">
                {plan.conflicts.map((c) => (
                  <li key={c.denom}>
                    {chipLabel(c.denom)}: short by {c.short}
                  </li>
                ))}
              </ul>
              <div className="text-[11px] text-ink-400 mt-2">
                Either trim the tournament starting stack, lower the cash buy-in, or grab more chips before play starts.
              </div>
            </div>
          </div>
        </Card>
      ...
      {plan.cash && (
        <Card>
          <p className="label">Dealer float after reservations</p>
          <p className="text-xs text-ink-400 mb-3">
            What's left in the dealer's hand once tournament starting stacks are bagged.
          </p>
          <ChipBag
            entries={plan.cash.remaining}
            empty="Nothing left over — all chips spoken for."
            grayscaleZero
          />
        </Card>
      )}
